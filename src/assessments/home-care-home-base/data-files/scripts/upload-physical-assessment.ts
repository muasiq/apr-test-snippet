...
const worksheet = 'PhysicalAssessment';

async function main() {
	const doc = await clearSheet(homeCareHomeBaseDocId, worksheet);
	const allKeys = getAllKeys(physicalAssessment as QuestionBase[]);
...
async function addRows(sheet: GoogleSpreadsheetWorksheet) {
	const toAdd = (physicalAssessment as QuestionBase[]).map((question) => {
