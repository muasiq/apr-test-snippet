...
export const allPhysicalAssessmentQuestions: AllAssessmentQuestions = (
	physicalAssessment.map((pa) => {
		const { id, cleanedText, originalText, questionType, responses } = pa;
		const parsed = responses?.map((r) => {
			return { text: r.cleanedText, originalText: r.text, followup: r.followup };
		});
		return {
			id,
			text: cleanedText,
...
export const allAideCarePlanQuestions: AllAssessmentQuestions = (acp as QuestionBase[]).map(mapAideCarePlanQuestion);
