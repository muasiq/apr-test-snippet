...
import { ElaraConfig } from './index';
import { qaConfig } from './qaConfig';

const allQuestions = ElaraConfig.allQuestions.filter((q) => {
	if (q.type) {
...
describe('Ensure Elara Configuration is Valid', () => {
	...
	it('Should not have ids in one config and not the other', () => {
		...
		expect(idsInCompletedNotInQA).toEqual(['M2020']);
	});
	...
	it('All ids in config (and follow-ups) should be in datafile', () => {
		...
		for (const id of allQaConfigQuestionsAndFollowups.map((q) => q.id)) {
			const match = ElaraConfig.allQuestions.find((q) => q.id === id);
			if (!match) {
	...
	it('Should not have any top level questions in data file not in config', () => {
		...
		expect(missingFromConfigIds.length).toEqual(97);
	...
	it('Should not have follow ups in the config', () => {
		...
		expect(followUpsInConfig.length).toEqual(10);
	});
