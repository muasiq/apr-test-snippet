...
describe('Ensure Choice Configuration is Valid', () => {
	...
	it('Should not have ids in once config and not the other', () => {
	...
	it('All ids in config (and follow-ups) should be in datafile', () => {
		...
		for (const id of allQaConfigQuestionsAndFollowups.map((q) => q.id)) {
			const match = ChoiceConfig.allQuestions.find((q) => q.id === id);
			if (!match) {
	...
	it('Should not have any top level questions in data file not in config', () => {
		...
		expect(missingFromConfigIds.length).toEqual(77);
